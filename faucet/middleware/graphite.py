...
class GraphiteMiddleware(BaseMiddleware):
    ...
    def get_graphite(self, uri, system_name):
        if self.config:
            return graphitesend.init(fqdn_squash=True, system_name=system_name, prefix=uri, graphite_server=self.config["graphite_server"], graphite_port=self.config["graphite_port"])
        else:
